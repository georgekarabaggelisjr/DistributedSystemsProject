...
def submit_command(
        data: str = typer.Argument(..., help="Path to the input data file (.txt)"),
        mapper: str = typer.Argument(..., help="Path to the mapper code (.py or .class)"),
        reducer: str = typer.Argument(..., help="Path to the reducer code (.py or .class)")
):
    """
    Uploads files to MinIO and schedules a new Map-Reduce job.
    """
    client = JobClient(state_manager)
    typer.echo(f"📤 Submitting files: {data}, {mapper}, and {reducer}...")

    try:
        # Ενημέρωσε και τη μέθοδο στον JobClient να δέχεται 3 αρχεία
        job_id = client.submit_job(data, mapper, reducer)
